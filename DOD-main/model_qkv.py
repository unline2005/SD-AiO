...
def apply_affine(x, gamma, beta):
    """
    支持 [B, M, N] 或 [B, N, M] 的张量仿射变换：
    对最后一维长度为 N 的维度进行 gamma/beta 调整。
    """
    if x.ndim == 3 and gamma.ndim == 2 and x.shape[0] == gamma.shape[0]:
        if x.shape[2] == gamma.shape[1]:  # [B, M, N]
            gamma = gamma.unsqueeze(1)  # [B, 1, N]
            beta = beta.unsqueeze(1)
            return x * (1 + gamma) + beta
        elif x.shape[1] == gamma.shape[1]:  # [B, N, M]
            gamma = gamma.unsqueeze(-1)  # [B, N, 1]
            beta = beta.unsqueeze(-1)
            return x * (1 + gamma) + beta
    return x  # 维度不匹配直接跳过
...
def my_lora_fwd_sft(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
    ...
    if self.disable_adapters:
        ...
        for active_adapter in self.active_adapters:
            ...
            if not self.use_dora[active_adapter]:
                ...
                # ✅ 这里加上条件判断的仿射变换调用
                if isinstance(lora_A, torch.nn.Linear):
                    if hasattr(self, "gamma") and hasattr(self, "beta") and self.gamma is not None and self.beta is not None:
                        _tmp = apply_affine(_tmp, self.gamma, self.beta)
